	...
	if (event.key === 'Enter') {
		...
		fetchDomains().then(domains => {
			...
			if (/^(https?|ftp):\/\/[^\s/$.?#].[^\s]*$/.test(inputUrl)) {
				...
				document.getElementById('siteurl').src = scope + Ultraviolet.codec.xor.encode(inputUrl.includes('.') ? 'https://' + inputUrl : 'https://yandex.com/search/touch/?text=' + encodeURIComponent(inputUrl));
			}
		});
	}
});
